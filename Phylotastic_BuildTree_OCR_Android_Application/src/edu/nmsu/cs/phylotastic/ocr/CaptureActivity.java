...
import edu.nmsu.cs.phylotastic.ocr.camera.CameraManager;
import edu.nmsu.cs.phylotastic.ocr.camera.ShutterButton;
import edu.nmsu.cs.phylotastic.ocr.language.LanguageCodeHelper;
import edu.nmsu.cs.phylotastic.ocr.language.TranslateAsyncTask;
import edu.nmsu.cs.phylotastic.ocr.R;
import edu.nmsu.cs.phylotastic.webservices.*;
...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback, 
  ...
  public String sourceTextResult_WS;
  ...
  boolean handleOcrDecode(OcrResult ocrResult) {
    ...
    /**********************************************************************/
    /** Phylotastic Project : Send text to Web Services and get the tree **/
    /**********************************************************************/
    Log.i("ThanhNH Final Result", ocrResult.getText().replace("\n", " "));
    sourceTextResult_WS = ocrResult.getText().replace("\n", " ").toString();
    if (sourceTextResult_WS != null){
		try {
			AlertDialog.Builder builder1 = new AlertDialog.Builder(CaptureActivity.this);
			builder1.setTitle("OCR Content from Image");
			builder1.setMessage(sourceTextResult_WS.toString());
					...
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int which) {
							class ProgressTask extends AsyncTask<String, Void, Boolean> {
							    ...
							    private String sourceTextResult;
							   
							    
								public ProgressTask(CaptureActivity activity,String sourceTextResult) {
									dialog = new ProgressDialog(CaptureActivity.this);
									this.sourceTextResult = sourceTextResult;
								}
								...
								protected void onPreExecute() {
									...
									dialog.setMessage("Please wait to request Web Service and buid Tree");
									dialog.show();
								}
								@Override
								protected Boolean doInBackground(String... params) {
									try {
										...
										lstParams.add(this.sourceTextResult);
										WSExecutionClient client = new WSExecutionClient("http://128.123.177.21:5003/WSExecution/runWSFunctionWithWSDL_json", "http://128.123.177.13/WSRegistry/sites/default/files/wsdl/usecase2_workflow1_text.wsdl", lstParams, "getPhylogeneticTree");
								...
								protected void onPostExecute(Boolean result) {
									if (result == true && dialog != null) {
										dialog.dismiss();
										try {
											Log.i("ThanhNH", "Da request and response is " + phyloTree);
											...
											builder.setNeutralButton("View (Browser)",
													new DialogInterface.OnClickListener() {
														public void onClick(DialogInterface dialog, int which) {
															Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://128.123.177.13/Phylotastic_DisplayTree_Project/display_tree_onezoom_v1.html?uri=&tree_data=" + phyloTree + "&format=newick_text"));
															startActivity(browserIntent);
														}
											});
											builder.setPositiveButton("View (OneZoom)",
													new DialogInterface.OnClickListener() {
														public void onClick(DialogInterface dialog, int which) {
															/* Su dung Web Browser */
															//Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://128.123.177.13/Phylotastic_DisplayTree_Project/display_tree_onezoom_v1.html?uri=&tree_data=" + phyloTree + "&format=newick_text"));
															...
															/* Su dung Web View */
															String httpRequest = "http://128.123.177.13/Phylotastic_DisplayTree_Project/display_tree_onezoom_v1.html?uri=&tree_data=" + phyloTree + "&format=newick_text";
															Intent i = new Intent(getApplicationContext(), TreeViewerActivity.class);
															i.putExtra("tree_view_web_service",httpRequest);
															startActivity(i);	
														}
											});
											builder.show();
										} catch (Exception e) {
											Log.e("ThanhNH", e.getMessage());
										}
									} else {
										Log.e("Error", "Error");  
									}
						  ...
						  new ProgressTask(CaptureActivity.this, sourceTextResult_WS).execute();
